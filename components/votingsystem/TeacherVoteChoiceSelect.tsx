...
const TeacherVoteChoiceSelect = ({ project, participantsCount, onButtonSelect, ranking }) => {
    const [amountOfChoiceButtons, setAmountOfChoiceButtons] = useState(3);
    const [selectedButtonId, setSelectedButtonId] = useState(null);

    useEffect(() => {
        // Find matching projectId within ranking array
        const position = ranking.findIndex((item) => item?.projectId === project.project_id);
    ...
    const renderChoiceButtons = () => {
        ...
        for (let i = 1; i <= amountOfChoiceButtons; i++) {
            buttons.push(<TeacherVoteChoiceSelectButton key={i} id={i} buttonText={i} onSelect={handleButtonSelect} isSelected={selectedButtonId === i} />);
        }
        return buttons;
    };
