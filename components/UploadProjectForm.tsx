...
const UploadProjectForm = () => {
    ...
    const [previewImageTooltipVisible, setPreviewImageTooltipVisible] = useState(false);
    ...
    const handleYoutubeLinkChange = (event) => {
        ...
        if (newYoutubeLink.trim() !== '' && isValidYoutubeLink(newYoutubeLink)) {
            ...
            setAddedYoutubeLinks((prevAddedYoutubeLinks) => [...prevAddedYoutubeLinks, newYoutubeLink]);
        } else {
        ...
        <form onSubmit={handleSubmit} className="w-2/2 flex flex-col gap-4 items-cente p-5 rounded-xl mb-5 mt-5 border-2 border-purple-500">
            ...
            <div className="flex flex-col gap-2 items-start w-full relative">
                <label className="text-white flex items-top" htmlFor="title">
                    ...
                    {titleTooltipVisible && (
                        <div className="tooltip-right bg-gray-800  absolute right-6 text-white px-2 py-0 rounded-sm">Enter the project title</div>
                    )}
                </label>
            ...
            <div className="flex flex-col gap-2 items-start w-full relative">
                <label className="text-white flex items-top" htmlFor="tags">
                    ...
                    {tagsTooltipVisible && (
                        <div className="tooltip-right bg-gray-800  absolute right-6 text-white px-2 py-0 rounded-sm">
                            Which technologies were used to create this project,...search in the already existing list or add your own tags
                        </div>
                    )}
                </label>
                    ...
                    <div style={{ position: 'relative', width: '23%' }}>
                        <svg width="125" height="50" style={{ position: 'absolute', top: '0', left: '0', zIndex: '-1' }}>
                            <g id="Path_1279" data-name="Path 1279" transform="translate(124.136 42.749) rotate(180)" fill="none">
                                ...
                                <path
                                    d="M 106.5530700683594 2.001434326171875 L 2 2.228050231933594 L 2 40.74853897094727 L 122.1357421875 40.74853897094727 L 122.1357421875 13.58554840087891 L 106.5530700683594 2.001434326171875 M 107.2130889892578 -3.814697265625e-06 L 124.1357421875 12.58024978637695 L 124.1357421875 42.74853897094727 L 0 42.74853897094727 L 0 0.2323760986328125 L 107.2130889892578 -3.814697265625e-06 Z"
                                    stroke="none"
                                    fill="#fff"
                                />
                            </g>
                    ...
                    <div style={{ position: 'relative' }}>
                        <svg width="332" height="50">
                            <path
                                id="Path_1280"
                                data-name="Path 1280"
                                d="M1387.318,5930.639v-41.872l-107.349,1.128H1062v29.346l11.842,11.4Z"
                                transform="translate(-1061 -5887.757)"
                                fill="#202033"
                                stroke="#fff"
                                stroke-width="2"
                                opacity="0.66"
                    ...
                    <div style={{ position: 'relative' }}>
                        <svg width="100" height="50" style={{ position: 'absolute', top: '0', left: '0', zIndex: '0', pointerEvents: 'none' }}>
                            <g id="Path_1279" data-name="Path 1279" transform="translate(0 1.724)" fill="none">
                                ...
                                <path
                                    d="M 1.999992370605469 0.2755851745605469 L 1.999992370605469 39.90145111083984 L 91.363525390625 39.90145111083984 L 91.363525390625 14.11148071289062 L 76.81528472900391 0.2755851745605469 L 1.999992370605469 0.2755851745605469 M -7.62939453125e-06 -1.724414825439453 L 77.61447143554688 -1.724414825439453 L 93.363525390625 13.25349235534668 L 93.363525390625 41.90145111083984 L -7.62939453125e-06 41.90145111083984 L -7.62939453125e-06 -1.724414825439453 Z"
                                    stroke="none"
                                    fill="#fff"
                                />
                            </g>
                        </svg>
                        <button type="submit" onClick={handleTagChange} className=" text-white px-2 rounded-sm ml-1 mt-2">
                            Add Tag
                        </button>
                    </div>
                </div>
            ...
            <div className="flex flex-col gap-2 items-start w-full relative">
                ...
                <div className="flex w-full">
                    <div style={{ position: 'relative' }}>
                        <svg width="470" height="50">
                            <path
                                id="Path_1280"
                                data-name="Path 1280"
                                d="M1524.318,5931.394v-42.626l-152.557,1.148H1062v29.876l16.828,11.6Z"
                                transform="translate(-1061 -5887.761)"
                                fill="#202033"
                                stroke="#fff"
                                stroke-width="2"
                                opacity="0.66"
                    ...
                    <div style={{ position: 'relative' }}>
                        ...
                        <button type="submit" onClick={handleLinkChange} className=" text-white px-2 rounded-sm ml-1 mt-2">
                            Add Link
                        </button>
                    </div>
                </div>
                {linkError == true && <p className="text-red-500">This is not a valid link</p>}
                {/* Render the selected tags */}
                <div className="flex flex-wrap gap-2 rounded-md">
            ...
            <div className="flex flex-col gap-2 items-start w-full relative">
                <label className="text-white flex items-center" htmlFor="links">
                    ...
                    {linksYoutubeLinkVisible && (
                        <div className="tooltip-right o bg-gray-800  absolute right-6 text-white px-2 py-0 rounded-s">
                            Only youtube urls are allowed. It is possible to add multiple youtube videos
                        </div>
                    )}
                </label>
                <div className="flex w-full">
                    ...
                    <div style={{ position: 'relative' }}>
                        ...
                        <button type="submit" onClick={handleYoutubeLinkChange} className=" text-white px-2 rounded-sm ml-1 mt-2">
                            Add Link
                        </button>
                    </div>
                ...
                {youtubeLinkError == true && <p className="text-red-500">This is not a valid youtube link</p>}
                {/* Render the selected links */}
                <div className="flex flex-col gap-2 rounded-md">
                    {addedYoutubeLinks.map((link, index) => (
                        <span key={index} className="bg-white text-gray-600 px-3 py-1 rounded-xl">
                            {link}
                            <button type="button" onClick={() => handleRemoveYoutubeLink(index)} className="ml-2 text-red-500">
                                X
                            </button>
                        </span>
                    ))}
            ...
            <div className="flex flex-col gap-2 items-start w-full relative ">
                <label className="text-white flex items-top" htmlFor="pdfFile">
                    ...
                    {pdfFileVisible && (
                        <div className="tooltip-right o bg-gray-800  absolute right-6 text-white px-2 py-0 rounded-s">
                            Only pdf files are allowed, if possible compress the pdf file to make the size smaller
                        </div>
                    )}
                </label>
                <div style={{ position: 'relative' }}>
                    ...
                    <input
                        ...
                        ref={fileInputRef}
                    />
                    {pdfFileSelected ? <span className="text-gray-400">{pdfFile ? pdfFile.name : ''}</span> : null}
                </div>
            </div>
            <hr className="h-px my-3 bg-gray-200 border-0 w-full "></hr>
            <div className="flex flex-col gap-2 items-start w-full relative">
                <label className="text-white flex items-top" htmlFor="previewImage">
                    Project preview image: <span className="font-thin ml-3 text-pink-500">*</span>
                    <span />
                    <span
                        className="info-icon ml-1 cursor-pointer bg-gray-800 w-5 text-center rounded-sm  absolute right-0"
                    ...
                    {previewImageTooltipVisible && (
                        <div className="tooltip-right o bg-gray-800  absolute right-6 text-white px-2 py-0 rounded-s">
                            This is the first image that the visitor will see from your project. This image will be used on the card to preview your project.
                        </div>
                    )}
                </label>
                <input
            ...
            <div className="grid grid-cols-2 gap-3 w-full">
                ...
                {imageFiles.map((file, index) => {
                    ...
                    if (file.type.includes('image')) {
                        return (
                            <div key={index} className="relative h-40 overflow-hidden bg-gray-400 bg-opacity-10 rounded-lg">
                                <img
            ...
            {uploadStatus === 'error' && <p className="text-red-500">Upload failed. Please try again.</p>}
