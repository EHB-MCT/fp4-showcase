import { faPlus, faEdit, faF, faLink } from "@fortawesome/free-solid-svg-icons";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faLinkedin, faFacebook, faGithub, faInstagram, faGithubSquare, faYoutube, faTiktok } from "@fortawesome/free-brands-svg-icons";
...
export default function Profile() {
  ...
  const imgPlaceholder = "/images/profile_banner_placeholder.jpg";
  const [profileImagePlaceholder,setProfileImagePlaceholder] = useState(1);
  useEffect(() => {
    ...
    // to determine the number inside the name of the placeholder
    async function getRandomNumber() {
      var randomNumber = Math.floor(Math.random() * 5) + 1;
      setProfileImagePlaceholder(randomNumber)
    }
    fetchData();
    getRandomNumber();
  }, [id, user?.uid]);
    ...
    <>
      ...
      {userObject && (
        <div
          ...
          style={{
            backgroundImage: `url(${
              userObject?.bannerImageUrl || imgPlaceholder // Use the placeholder image if bannerImageUrl is not available or empty
            })`,
            backgroundSize: "cover",
            backgroundPosition: "center",
          }}
        >
          {/* Add a new div for the opacity background */}
          <div className={style.banner_overlay} />
          <div className={style.banner_header} style={{ position: "relative" }}>
            {isPersonalProfile && (
              <div className={style.editButtonContainer}>
                ...
                >
                  {" "}
                  <FontAwesomeIcon icon={faEdit} width={25} height={25} />
                </button>
              </div>
            )}

            {userObject && (
              <div className={style.profileSocialsContainer}>
                {userObject.socials?.map((social, index) => (
                  <a href={social} key={index}>
                    {social.includes("linkedin.com") && (
                      <FontAwesomeIcon
                        icon={faLinkedin}
                        className={style.fontawesomeSocialsIcon}
                      />
                    )}
            ...
            {userObject && (
              <img
                src={
                  userObject.previewImageUrl
                    ? userObject.previewImageUrl
                    : `/images/profile_image_placeholder_${profileImagePlaceholder}.png`
                }
                alt="profile image"
            ...
            <div>
              <h1 className={style.usernameContainer}>{userObject.username}</h1>

              {userObject &&
                userObject.interests &&
                userObject.interests.length > 0 && (
                  <>
                    <div className="flex flex-row gap-2 absolute bottom-0">
                      <p style={{ marginRight: "25px" }}>My interests:</p>
                      <div className={style.profileInterestsContainer}>
                        {userObject.interests.map((tag, index) => (
                          <p key={index}>{tag}</p>
                        ))}
                      </div>
                    </div>
      ...
      <div className={`${style.content_wrapper} `}>
        <div className={`${style.content} containerWidth`}>
          <div className={style.about_me}>
            <h1>About me</h1>
            {userObject &&
              (!userObject.aboutMe || userObject.aboutMe === "" ? (
