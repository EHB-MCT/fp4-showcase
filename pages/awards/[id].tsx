import { FieldValue, collection, doc, getDoc, getDocs, setDoc, updateDoc } from 'firebase/firestore';
        ...
        const fetchVotes = async () => {
            try {
                ...
                if (votes) {
                    ...
                    setHasVoted(true);
                    setFirst({ awardId: id, projectId: votes.order.first, docentId, selectedButtonId: 1 });
                    setSecond({ awardId: id, projectId: votes.order.second, docentId, selectedButtonId: 2 });
                    setThird({ awardId: id, projectId: votes.order.third, docentId, selectedButtonId: 3 });
                }
            } catch (e) {
                console.log(e);
            }
        ...
        try {
            ...
            if (selectedProject) {
                ...
                if (!participatingProject || participatingProject.project_id !== selectedProject.project_id) {
                    ...
                    const previousProject = projects.find((project) => project.awardId === id);
                    if (previousProject) {
                        const projectRef = doc(firestore, 'projects', previousProject.project_id);
                ...
                <div>
                    <div className={`${styles.awardProjects} flex flex-col`}>
                        ...
                        <div className="flex gap-5">
                            <h1>Projects</h1>
                            {userData && userData.role === 'student' && currentDate < specificDate && !hasParticipated && (
                            ...
                            {userData && userData.role === 'docent' && currentDate > specificDate && currentDate < docentVoteDate && !startVotingTeacher && (
                                <ButtonPink title="Start Voting" color="white" onClick={handleStartVotingButtonClick} />
                            )}
                                ...
                                {userProjects.map((project) => (
                                    ...
                                    >
                                        <div className="flex items-center">
                                            <img src={project.previewImageUrl} className="w-24 h-24 rounded-sm mr-2" alt="Project Preview" />
