import { FieldValue, addDoc, collection, doc, getDoc, getDocs, setDoc, updateDoc } from 'firebase/firestore';
...
export default function Award() {
    ...
    const [isChangeParticipationModalOpen, setIsChangeParticipationModalOpen] = useState(false);
    ...
    const [superAwardProjects, setSuperAwardProjects] = useState([]);
    ...
    const [winnerAnnouncementDeadline, setWinnerAnnouncementDeadline] = useState(null);
    ...
    useEffect(() => {
        const fetchDates = async () => {
            try {
                ...
                setWinnerAnnouncementDeadline(new Date(deadlines.winnerAnnouncement.seconds * 1000));
            } catch (error) {
    ...
    useEffect(() => {
        ...
        const fetchAllProjects = async () => {
            try {
                if (!id) return;
                // hardcoded super award id
                if (id === 'HTCtIirzCJFy4Cac4FUD') {
                    const projects = await getAllProjects();
                    setProjects(projects);
                    const projectsWithSuperAwardId = projects.filter((project) => project.superAwardId === id);
                    setProjectsToVoteOn(projectsWithSuperAwardId);
                } else {
                    const projects = await getAllProjects();
                    setProjects(projects);
                    const projectsWithAwardId = projects.filter((project) => project.awardId === id);
        ...
        try {
            ...
            if (selectedProject) {
                ...
                if (participatingProject) {
                    ...
                    const projectRef2 = doc(firestore, 'projects', selectedProject.project_id);
                    ...
                    const projectsToVoteOn = updatedProjects.filter((projects) => projects.awardId === id);
    ...
    const handleConfirmParticipationButtonClick = async () => {
        try {
            ...
            if (selectedProject) {
                ...
                // If the selected project is not already participating in this award
                if (!participatingProject || participatingProject.project_id !== selectedProject.project_id) {
                    const projectRef = doc(firestore, 'projects', selectedProject.project_id);
                    ...
                    const updatedProjects = await getAllProjects();
                    setProjects(updatedProjects);
                    const projectsToVoteOn = updatedProjects.filter((project) => project.awardId === id);
                    setProjectsToVoteOn(projectsToVoteOn);
                } else {
                    console.log('Selected project is already participating in this award');
                }
            }
        } catch (error) {
            console.error('Error confirming participation:', error);
    ...
    const handleConfirmWithdrawButtonClick = async () => {
        ...
        // The request was successful, update the projects by fetching them again
        // const updatedProjects = await getAllProjects();
        const updatedProjects = projects.filter((project) => project.project_id !== participatingProject.project_id && project.awardId === id);
    ...
    useEffect(() => {
        // ...
        if (!user) return;
        const hasParticipated = projects.some((project) => project.awardId === id && project.uid === user.uid);
                ...
                <div className="containerWidth">
                    ...
                    <div>
                        <div className="flex flex-col">
                            {winner && hasAwardEnded && userData != null && userData.role === 'admin' && currentDate < winnerAnnouncementDeadline && (
                                <div>
                                    <h1>Award Winner</h1>
                                    <div className=" mt-5 mb-5 ">{winner && <ProjectCard project={winner} />}</div>
                                </div>
                            )}
                            {winner && hasAwardEnded && currentDate > winnerAnnouncementDeadline && (
                                <div>
                                    <h1>Award Winner</h1>
                                    <div className=" mt-5 mb-5 ">{winner && <ProjectCard project={winner} />}</div>
                                </div>
                        ...
                        <div className={`${styles.awardProjects} flex flex-col`}>
                            {currentDate < adminVoteDeadline && currentDate > docentVoteDeadline && userData && userData.role === 'admin' && (
                                <div className="">
                                            ...
                                            <div className="customGrid mt-5 mb-5">
                                                <select
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        const selectedProject = top3Projects.find((project) => project.project_id === selectedProjectId);
                                                ...
                                                <div>
                                                    <ButtonPink title="Submit" color="white" onClick={handleSubmitWinner} />
                                                </div>
                                            </div>
                                        </div>
                                    )}
                            ...
                            <div className="flex gap-2">
                                ...
                                {userData && userData.role === 'student' && currentDate < participateDeadline && !hasParticipated && (
                                    <ButtonPink title="Participate" color="white" onClick={handleParticipateButtonClick} />
                                )}

                                {userData && userData.role === 'student' && currentDate < participateDeadline && hasParticipated && (
                                    <>
                                        <ButtonPink title="Change" color="white" onClick={handleChangeParticipateButtonClick} />
                                        <ButtonPink title="Remove" color="white" onClick={handleWithdraweButtonClick} />
                                    </>
                                )}
                                ...
                                {userData &&
                                    ...
                                    !startVotingTeacher &&
                                    !hasVoted && <ButtonPink title="Start Voting" color="white" onClick={handleStartVotingButtonClick} />}
                                {startVotingTeacher && <ButtonPink title="Confirm Vote" color="white" onClick={handleConfirmVotingButtonClick} />}

                                {startVotingTeacher && <ButtonPink title="Cancel" color="white" onClick={handleCancelVotingButtonClick} />}
                            </div>
                        </div>
                        {startVotingTeacher && (
                            <div className="bg-black p-2 w-fit">Vote the best projects by order. 1 = first choice, 2 = second choice, 3 = third choice</div>
                        )}
                    </div>
                        ...
                        <div className="bg-black  p-2 w-fit mt-3 opacity-80">
                            <p className="font-light ml-2">
                                ...
                                <span className="text-fuchsia-600">
                                    &#34;
                                    {projects.find((project) => project.awardId === id && project.uid == user.uid)?.title}
                                    &#34;
                                </span>
                                . You can still change your submission or remove your submission.
                            </p>
                        </div>
                    )}

                    <div className="customGrid mt-5 mb-5 gap-3">
                        {award &&
                            projects.length > 0 &&
                            projectsToVoteOn.map((project, index) => {
                                return (
                                    <>
                                        <div className="relative" key={index} style={startVotingTeacher ? { marginBottom: '7rem' } : { marginBottom: '2rem' }}>
                                            <ProjectCard key={project.project_id} project={project} />
                                            {startVotingTeacher && (
                    ...
                    {projectsToVoteOn.length === 0 && award && <p>No projects uploaded to {award.title}... Come back at a later time.</p>}
                    {isWithdrawModalOpen && (
                        ...
                        <div className={styles.participationModalContainer}>
                            ...
                            <div className={styles.participationModalHeader}>
                                ...
                                <h2 className={styles.participationModalHeaderTitle}>
                                    Participate
                                    <span className={styles.participationModalHeaderTitleSpan}> - select your project</span>
                                </h2>
                    ...
                    {isChangeParticipationModalOpen && (
                        <div className={styles.participationModalContainer}>
                            ...
                            <div className={styles.participationModalHeader}>
                                ...
                                <ButtonPink title="Confirm" color="white" onClick={handleConfirmChangeParticipationButtonClick} />
                            </div>
                        </div>
                    )}
                </div>
